...
export default function KeywordDeduplicator() {
  ...
  const handleManualProcess = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (!error) {
        // Check the error state variable directly
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Deduplicated keywords exported successfully.',
        variant: 'default',
      });
    } catch (exportError) {
          ...
          <div className="space-y-4 rounded-lg border p-4">
            ...
            {products.length > 0 && !isLoading && (
              <p className="text-xs text-green-600 mt-1">
                {products.length} product(s) loaded and processed.
              </p>
            )}
          </div>
