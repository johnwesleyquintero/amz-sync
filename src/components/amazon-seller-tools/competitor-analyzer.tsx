...
export default function CompetitorAnalyzer() {
  ...
  const analyzeCompetitor = useCallback(() => {
    ...
    if (!sellerData) {
      ...
      toast({ title: 'Missing Data', description: 'Seller data CSV is required.', variant: 'destructive' });
      return;
    }
    if (competitorData.length === 0) {
      setError('Competitor data is missing. Please upload the competitor CSV file.');
      toast({ title: 'Missing Data', description: 'Competitor data CSV is required.', variant: 'destructive' });
      return;
    }
    if (selectedMetrics.length === 0) {
      setError('No metrics selected. Please choose at least one metric to compare.');
      toast({ title: 'Missing Selection', description: 'Please select metrics to compare.', variant: 'destructive' });
      return;
    }
        ...
        selectedMetrics.forEach((metric) => {
          ...
          if (value !== undefined && value !== null && value !== '') {
            ...
            if (!isNaN(numValue)) {
              ...
              console.warn(`Invalid numeric value for metric '${metric}' in competitor ${competitorName}: ${value}`);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Upload CSV files for your product and competitor products to compare key metrics.
          The ASIN input below is currently for reference only and does not fetch data.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4 rounded-lg border p-4">
           ...
           <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
            {/* Seller CSV Input */}
            <div>
              <Label htmlFor="seller-csv" className="flex items-center gap-1 mb-1 text-sm font-medium">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="text-xs max-w-xs">
                        ...
                        Upload a CSV containing data for your own product. Only the first valid row will be used.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              ...
              <div className="flex items-center gap-2">
                 ...
                 <Input
                    ...
                    disabled={isLoading}
                 />
                 <SampleCsvButton dataType="competitor" fileName="sample-seller-data.csv" size="sm" buttonText="Sample" />
              </div>
            ...
            <div>
              <Label htmlFor="competitor-csv" className="flex items-center gap-1 mb-1 text-sm font-medium">
                Competitor Data (CSV)
                <TooltipProvider delayDuration={100}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                    ...
                    <TooltipContent>
                      <p className="text-xs max-w-xs">
                         Required columns: {REQUIRED_COLUMNS.join(', ')}.
                         Upload a CSV containing data for one or more competitor products.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
               ...
               <div className="flex items-center gap-2">
                 ...
                 >
                    <Upload className="mr-2 h-4 w-4" />
                    {competitorData.length > 0 ? `${competitorData.length} Competitor(s) Loaded` : 'Choose Competitor File...'}
                 </Button>
                 <Input
                    id="competitor-csv"
                    type="file"
                    ...
                    disabled={isLoading}
                 />
                 <SampleCsvButton dataType="competitor" fileName="sample-competitors-data.csv" size="sm" buttonText="Sample" />
               </div>
               {competitorData.length > 0 && <p className="text-xs text-green-600 mt-1">{competitorData.length} competitor(s) loaded.</p>}
            </div>
          </div>
           {/* Loading Indicator */}
        ...
        <div>
          <Label htmlFor="asinInput" className="text-sm font-medium">Competitor ASIN or Niche (Reference Only)</Label>
          <Input
            id="asinInput"
            value={asinInput}
            ...
            disabled={isLoading}
          />
           <p className="text-xs text-muted-foreground mt-1">This input is for context and does not currently fetch data.</p>
        </div>
                  ...
                  >
                    ...
                    {selectedMetrics.map((metric) => (
                      <Line
                        ...
                        type="monotone"
                        dataKey={metric}
                        name={metric.split('_').map(w => w[0].toUpperCase() + w.slice(1)).join(' ')} // Format legend name
                  ...
                  >
                    ...
                    {selectedMetrics.map((metric) => (
                      <Bar
                        key={metric}
                        dataKey={metric}
                        name={metric.split('_').map(w => w[0].toUpperCase() + w.slice(1)).join(' ')}
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {selectedMetrics.map((metric) => (
                        <TableHead key={metric}>{metric.split('_').map(w => w[0].toUpperCase() + w.slice(1)).join(' ')}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {chartData.map((row) => (
                      <TableRow key={row.name}>
                        ...
                        {selectedMetrics.map((metric) => (
                          <TableCell key={metric}>{row[metric] !== undefined ? row[metric] : 'N/A'}</TableCell>
                        ))}
                      </TableRow>
                    ))}
                  </TableBody>
