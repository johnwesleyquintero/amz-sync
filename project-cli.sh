...
log_error() {
    ...
    if [[ -f "$caller_file" && -r "$caller_file" ]]; then
        ...
        echo -e "${timestamp} [${error_type}] [Source: ${caller_file##*/}:${caller_line}] [Script: ${current_file}] ${error_context}\n  Context:\n${code_snippet}\n  Stack:\n${FUNCNAME[*]}" >> "$ERROR_LOG_FILE"
    else
        echo -e "${timestamp} [${error_type}] [Source: ${caller_file##*/}:${caller_line}] [Script: ${current_file}] ${error_context}\n  Stack:\n${FUNCNAME[*]}" >> "$ERROR_LOG_FILE"
    fi
    ...
    while true; do
        ...
        case $choice in
            ...
            2|"t") {
                ...
                npm run test 2>&1 | grep -i "error\|fail" > "$ERROR_LOG_FILE"
