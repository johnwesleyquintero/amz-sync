...
const CONFIG = {
  ...
  generateReport: true,
  // Report path
  reportPath: path.join(process.cwd(), '.wescore', 'reports', 'package-report.json'),
...
async function runCommand(command, description, ignoreExitCode = false) {
  ...
  try {
    ...
    if (ignoreExitCode && error.stdout) {
      spinner.warn(
        chalk.yellow(`${description} - Completed with issues (exit code ${error.code})`)
      );
...
function safeJsonParse(jsonString, context) {
  try {
    ...
    console.error(chalk.red(`Error parsing JSON for ${context}: ${error.message}`));
...
async function runSecurityAudit() {
  ...
  const command = `npm audit ${flags.join(' ')} --audit-level=${CONFIG.auditLevel}`;
  ...
  const totalVulns = summary.total || Object.keys(auditData.vulnerabilities || {}).length; // Fallback count

  if (totalVulns > 0) {
    console.log(
      chalk.red(`Found ${totalVulns} vulnerabilit(ies) at or above '${CONFIG.auditLevel}' level:`)
    );
    console.log(
      chalk.red(
    ...
    console.log(chalk.green(`No vulnerabilities found at or above '${CONFIG.auditLevel}' level.`));
  }
...
async function checkVersionConflicts() {
  ...
  if (!lsData || !lsData.dependencies) {
    console.log(chalk.yellow('Could not parse dependency tree data or no dependencies found.'));
  ...
  if (conflictCount > 0) {
    console.log(chalk.yellow(`Found ${conflictCount} package(s) with multiple resolved versions:`));
    Object.entries(conflicts).forEach(([name, data]) => {
      console.log(chalk.yellow(`  - ${name}: Versions [${data.versions.join(', ')}]`));
      // Optionally log paths if needed for debugging (can be verbose)
      // data.paths.forEach(p => console.log(chalk.gray(`    ${p}`)));
    });
  } else {
    console.log(chalk.green('No duplicate package versions found in the dependency tree.'));
  }
...
function checkStrictPinning() {
  ...
  try {
    ...
    if (violations.length > 0) {
      console.log(chalk.yellow(`Found ${violations.length} non-pinned dependenc(ies):`));
      violations.forEach(v =>
        console.log(chalk.yellow(`  - ${v.name}: "${v.version}" (${v.type})`))
      );
    } else {
      console.log(chalk.green('All dependencies appear to be strictly pinned.'));
    }
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    if (overallStatus === 'success') {
      ...
      console.log(chalk.bold.yellow('⚠️ Package checks completed with warnings. Review output.'));
    } else {
      console.log(chalk.bold.red('❌ Package checks failed. Review output and report.'));
      process.exitCode = 1; // Set exit code to indicate failure
    }
  } catch (error) {
    console.error(chalk.bold.red('\n--- Critical Error ---'));
    console.error(chalk.red(`Package management check failed: ${error.message}`));
