...
async function updateProjectTracker() {
  log('info', `Starting project tracker update for: ${C.filePath(TRACKER_PATH)}`);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith(COMPLETED_SECTION_HEADING)) {
      ...
      log('info', `Found '${COMPLETED_SECTION_HEADING}' starting at line ${i + 1}.`);
    ...
    if (completedSectionStartIndex === -1 || i < completedSectionStartIndex) {
      ...
      if (line.startsWith('### ')) {
        ...
        log('info', `Processing under section: ${C.heading(currentSectionHeading)}`);
      }
  ...
  if (completedTasksBySection.size === 0) {
    log('warn', 'No new completed tasks found to move. File remains unchanged.');
  ...
  for (
    let i = 0;
    i < (completedSectionStartIndex === -1 ? lines.length : completedSectionStartIndex);
    i++
  ) {
  ...
  const adjustedCompletedSectionEndIndex =
    completedSectionStartIndex + (completedSectionEndIndex - completedSectionStartIndex);
  for (let i = completedSectionStartIndex + 1; i < adjustedCompletedSectionEndIndex; i++) {
  ...
  if (completedSectionStartIndex === -1) {
    if (finalNewLines.length > 0 && finalNewLines[finalNewLines.length - 1].trim() !== '')
      finalNewLines.push('');
  ...
  if (completedSectionStartIndex !== -1) {
    for (let i = completedSectionStartIndex + 1; i < completedSectionEndIndex; i++) {
      finalNewLines.push(lines[i]);
    }
  }
  ...
  if (finalContent === fileContent) {
    log('warn', 'File content did not change after processing. No write needed.');
    return;
  }

  try {
    await fs.writeFile(TRACKER_PATH, finalContent, 'utf-8');
    log('success', `Successfully updated tracker file: ${C.filePath(TRACKER_PATH)}`);
  } catch (error) {
