...
async function playSound(soundType) {
  ...
  if (soundPath) {
    try {
      ...
      if (err.code === 'ENOENT') {
        log('DEBUG', `Sound file not found: ${soundPath}. Cannot play '${soundType}' sound.`);
      } else {
        log('DEBUG', `Failed to play sound '${soundType}' from ${soundPath}: ${err.message}`);
      }
    }
  }
}
...
function showHackerHeader() {
  ...
  console.log(BRO_CONFIG.colors.header('<<< BRO-EDITION EXTREME - v3.14 - ENGAGE! >>>'));
  console.log(
...
function log(level, message, checkName = '') {
  ...
  const checkTag = checkName ? BRO_CONFIG.colors.checkName(`[${checkName}]`) : '';
...
async function runEpicCheck(check, index, total) {
  ...
  try {
    ...
    if (result.exitCode === 0) {
      ...
      const failureError = new Error(`Command failed with exit code ${result.exitCode}`);
...
async function runWescore() {
  ...
  try {
    ...
    if (checks.length === 0) {
      log('WARN', 'No checks enabled in .wescore.json. Nothing to do, I guess? 🤷');
    ...
    const runPromises = async () => {
      if (config.runMode === 'parallel') {
        ...
        const promises = checks.map((check, i) => runEpicCheck(check, i + 1, totalChecks));
    ...
    // Determine exit code
    const failedCount = results.filter(r => r.status === 'failed' || r.status === 'errored').length;
...
function showFinalReport(results, passed, totalRun, totalDuration) {
  ...
  if (failed > 0) {
    ...
    results
      ...
      .forEach(check => {
        const statusColor =
          check.status === 'errored' ? BRO_CONFIG.colors.error : BRO_CONFIG.colors.warn;
        const exitInfo = check.exitCode ? ` (Exit: ${check.exitCode})` : '';
        const errorMsg = check.error ? `: ${check.error.shortMessage || check.error.message}` : '';
        console.log(
    ...
    log('ERROR', "Scroll up to see the glorious details of each failure. Fix 'em, champ!");
  }

  log('INFO', `Wescore Bro-Edition signing off. ${success ? 'Stay frosty.' : 'Try harder, n00b.'}`);
}
  ...
  try {
    ...
    const enabledChecks = config.checks.filter((c, index) => {
      ...
      if (!c.name || typeof c.name !== 'string') {
        log('WARN', `Check at index ${index} is missing a 'name' (string). Skipping.`);
        return false;
      }
      if (!c.command || typeof c.command !== 'string') {
        log('WARN', `Check "${c.name}" is missing a 'command' (string). Skipping.`);
        return false;
      }
    ...
    return {
      runMode: ['parallel', 'sequential'].includes(config.runMode) ? config.runMode : 'sequential', // Default to sequential
...
runWescore().catch(e => {
  ...
  console.error('\n' + chalk.bgRed.whiteBright.bold('! C A T A S T R O P H I C   F A I L U R E !'));
