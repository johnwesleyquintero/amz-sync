  ...
  try {
    ...
    config.checks.forEach((check, index) => {
      if (!check.name || typeof check.name !== 'string') {
        throw new Error(`Config Error! Check at index ${index} needs a valid 'name', dude!`);
      }
      if (!check.command || typeof check.command !== 'string') {
        throw new Error(`Config Error! Check "${check.name}" is missing a 'command'. Not cool!`);
      }
    });
...
function generateRetroSummary(data) {
  ...
  const textLine = (label, value) => {
    const formattedValue = value.length > 40 ? value.slice(0, 37) + '...' : value; // Truncate long values
  ...
  const failedChecksLines = data.failedCheckNames.map(name => textLine('  - Failed', name));
...
▀▄▀▄▀ ██▄ ▄█ █▄▄ █▄█ █▀▄ ██▄   █▀█ █░▀█ █▄▀   █▄▄ █▀█ █ █▄▄ █▄▄
  ...
  const executeCheck = async (check, index, total) => {
    ...
    try {
      ...
      if (result.exitCode === 0) {
        ...
        log('ERROR', `  ${THEME_COLORS.red}Output Log:${THEME_COLORS.reset}\n--- Error Output ---`);
        if (checkResult.stderr && checkResult.stderr.trim()) {
          log(
            'ERROR',
  ...
  if (config.config.runMode === 'parallel') {
    ...
    const checkPromises = allChecks.map((check, i) => executeCheck(check, i, allChecks.length));
  ...
  if (failedChecksData.length > 0) {
    ...
    failedChecksData.forEach(check => {
      log('ERROR', `  ${THEME_COLORS.magenta}▼ ${check.name}${THEME_COLORS.reset}`);
    });
    ...
    failedChecksData.forEach(check => {
      ...
      const errorOutput = (check.stderr || check.stdout || 'No specific output captured.').trim();
      console.log(`  ${THEME_COLORS.grey}Review the error log snippet below:${THEME_COLORS.reset}`);
      console.log(
        `${THEME_COLORS.red}----------[ Error Snippet Start ]----------${THEME_COLORS.reset}`
      );
      // Indent snippet lines
      console.log(`  ${truncateOutput(errorOutput, 15).split('\n').join('\n  ')}`);
      console.log(
...
`);
  ...
  log('INFO', `Shutdown sequence initiated... Exiting with code ${checksFailed > 0 ? 1 : 0}.`);
