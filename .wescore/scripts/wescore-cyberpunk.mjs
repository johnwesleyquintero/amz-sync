...
function showCyberHeader() {
  ...
  console.log(
    CYBER_CONFIG.colors.header('<<< CYBERPUNK EDITION // v2077.42 // SYSTEM CHECK INITIATED >>>')
  );
  console.log(
...
function log(level, message, checkName = '') {
  ...
  const checkTag = checkName ? CYBER_CONFIG.colors.checkName(`[${checkName}]`) : '';
...
async function runSubroutine(check, index, total) {
  ...
  try {
    ...
    if (result.exitCode === 0) {
      ...
      const failureError = new Error(`Subroutine failed with exit code ${result.exitCode}`);
...
async function runCyberCheck() {
  ...
  try {
    ...
    if (checks.length === 0) {
      log('WARN', 'No active protocols found in .wescore.json. System idle. 🤷');
    ...
    const executeProtocols = async () => {
      if (config.runMode === 'parallel') {
        log('INFO', 'Engaging parallel processing cores... Maximum bandwidth! 🔥');
        const promises = checks.map((check, i) => runSubroutine(check, i + 1, totalChecks));
        ...
        for (let i = 0; i < totalChecks; i++) {
          ...
          if (result.status === 'nominal') {
            ...
            log('WARN', `🚨 ABORT TRIGGERED! Protocol '${check.name}' glitched. Disconnecting! 🚨`);
...
function showFinalSitrep(results, nominal, totalRun, totalDuration) {
  ...
  if (allNominal) {
    ...
    log('SUCCESS', 'All protocols executed without anomaly. Clean run, netrunner. 😎');
    playSound('successAll');
  } else {
    renderFiglet('ANOMALY DETECTED', 'Bloody', CYBER_CONFIG.colors.error);
    log('ERROR', `Run incomplete or corrupted: ${anomalies} protocol(s) reported anomalies.`);
  ...
  const summaryBox = boxen(
    ...
    ].join('\n'),
    {
      ...
      borderColor: allNominal ? CYBER_CONFIG.colors.success('') : CYBER_CONFIG.colors.error(''),
      title: '[ SITREP ]',
      titleAlignment: 'center',
  ...
  if (anomalies > 0) {
    ...
    results
      ...
      .forEach(check => {
        const statusColor =
          check.status === 'critical_error' ? CYBER_CONFIG.colors.error : CYBER_CONFIG.colors.warn;
        const statusIcon = check.status === 'critical_error' ? '☣️' : '❗';
        const exitInfo = check.exitCode ? ` (Exit: ${check.exitCode})` : '';
        const errorMsg = check.error ? `: ${check.error.shortMessage || check.error.message}` : ''; // Show execa error message if present
        console.log(
    ...
    log('ERROR', 'Review the logs above for detailed anomaly reports. Patch the system, choom.');
  }
...
async function loadConfig() {
  ...
  try {
    ...
    if (!configData.checks || !Array.isArray(configData.checks)) {
      throw new Error("Config shard missing 'checks' array protocol. Data incomplete.");
    }

    const enabledChecks = configData.checks.filter((c, index) => {
      if (typeof c !== 'object' || c === null) {
        log('WARN', `Protocol at index ${index} is corrupted (not an object). Skipping.`);
        return false;
      }
      if (!c.name || typeof c.name !== 'string') {
        log('WARN', `Protocol at index ${index} lacks 'name' identifier. Skipping.`);
        return false;
      }
      if (!c.command || typeof c.command !== 'string') {
        log('WARN', `Protocol "${c.name}" lacks 'command' subroutine. Skipping.`);
        return false;
      }
    ...
    throw new Error(`Failed to load or validate config shard: ${error.message}`);
  }
}
...
runCyberCheck().catch(e => {
  ...
  console.error(
    '\n' + CYBER_CONFIG.colors.critical('! C A T A S T R O P H I C   M A L F U N C T I O N !')
  );
  console.error(
    CYBER_CONFIG.colors.error('The Wescore runner flatlined! Core meltdown imminent! 🔥')
  );
