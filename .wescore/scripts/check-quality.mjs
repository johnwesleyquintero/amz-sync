...
function configureLogLevel() {
  ...
  const debugFlags = process.argv.includes('--debug') || process.argv.includes('-d');
  ...
  if (currentLogLevel < LOG_LEVELS.SILENT) {
    log('DEBUG', `Log level set to ${C.bold(determinedLevel)} (Value: ${currentLogLevel})`);
  }
}
...
async function loadConfig() {
  log('INFO', `${ICONS.config} Loading configuration from ${C.filePath(CONFIG_PATH)}...`);
  try {
    ...
    if (
      ...
      throw new Error('Core `config` or `config.visuals` structure is missing or invalid.');
    }
    ...
    config.checks.forEach((check, index) => {
      ...
      if (check.cwd !== undefined && (typeof check.cwd !== 'string' || !check.cwd.trim())) {
        throw new Error(`${checkId}: 'cwd' must be a non-empty string if provided.`);
      }
...
async function executeCheck(check, index, total) {
  ...
  try {
    ...
    checkResult.output = error.all || `${checkResult.stdout}\n${checkResult.stderr}`;
...
async function printSummary(results, startTime, endTime, gitCommitHash) {
  ...
  const statusText = allPassed ? C.success('ALL CHECKS PASSED') : C.error('CHECKS FAILED');
  const statusIcon = allPassed ? ICONS.success : ICONS.failure;
  const boxBorderColor = allPassed ? 'green' : 'red';

  renderFiglet(allPassed ? 'Checks Passed' : 'Checks Failed', allPassed ? C.success : C.error);
...
async function printFailureDetails(failedChecks) {
  ...
  console.log(`2. Identify the root cause (e.g., lint errors, test failures, command issues).`);
  console.log(`3. Fix the underlying issues in your codebase or configuration.`);
  console.log(`4. Re-run checks using your designated command (e.g., ${C.command('npm run cq')}).`);
...
async function runQualityChecks() {
  ...
  try {
    ...
    if (!(await loadConfig())) {
      log('ERROR', `${ICONS.error} Exiting due to critical configuration loading errors.`);
    ...
    if (enabledChecks.length === 0) {
      log('WARN', `${ICONS.warning} No enabled checks found in configuration. Nothing to execute.`);
      printSeparator();
    ...
    log('INFO', `Finished running ${results.length} checks. Failures: ${checksFailedCount}.`);
