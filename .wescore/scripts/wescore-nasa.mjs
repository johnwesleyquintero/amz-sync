...
function log(level, message) {
  ...
  const indentedMessage = message
    .split('\n')
    .map((line, index) => (index === 0 ? line : `                       ${line}`)) // Align with prefix end
    .join('\n');
...
function generateMissionReport(data) {
  ...
  const textLine = (label, value, valueColor = THEME_COLORS.white) => {
    ...
    const formattedValue = cleanValue.length > 45 ? cleanValue.slice(0, 42) + '...' : cleanValue;
 ...
 \|____________|/
  ...
  if (failedChecksData.length > 0) {
    ...
    failedChecksData.forEach(check => {
      log('ERROR', `  ${THEME_COLORS.orange}↳ ${check.name}${THEME_COLORS.reset}`);
    });
