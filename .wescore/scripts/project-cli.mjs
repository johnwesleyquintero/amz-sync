...
const REQUIRED_PROJECT_FILES = ['package.json', 'tsconfig.json', 'next.config.js']; // Adjust as needed

const COMMANDS = {
  reset: 'Reset the project environment (clean build artifacts and node_modules, then reinstall)',
...
async function writeLog(message, level = 'INFO') {
  ...
  try {
    ...
    console.error(chalk.red(`Failed to write to log file ${logPath}: ${error.message}`));
  } finally {
    switch (level) {
      case 'ERROR':
...
async function getCommandVersion(command) {
  try {
    ...
    if (error.code === 'ENOENT') {
      writeLog(`Command not found: ${command}. Ensure it is installed and in your PATH.`, 'WARN');
    }
    return null;
  }
}
    ...
    return await retryOperation(
      async () => {
        ...
        if (missingFiles.length > 0) {
          writeLog(`Missing required project files: ${missingFiles.join(', ')}`, 'ERROR');
          return false;
        }
...
async function checkGlobalPackages() {
  ...
  for (const pkg of REQUIRED_GLOBAL_PACKAGES) {
    try {
      ...
      if (
        ...
        writeLog(`Missing required global package: ${pkg} during global package check`, 'WARN');
      } else {
        // Log other errors
        writeLog(`Error checking global package ${pkg}: ${error.message}`, 'ERROR');
  ...
  if (missingPackages.length > 0) {
    writeLog(`Missing required global packages: ${missingPackages.join(', ')}`, 'ERROR');
  ...
  try {
    ...
    try {
      ...
      if (parseError instanceof SyntaxError) {
        writeLog(`Failed to parse package.json: SyntaxError - ${parseError.message}`, 'ERROR');
    ...
    // Git info
    const gitBranch = await execPromise('git rev-parse --abbrev-ref HEAD').then(({ stdout }) =>
      stdout.trim()
    );
    ...
    console.log(`Node Version Required: ${chalk.green(pkg.engines?.node || 'Not specified')}`);
    ...
    console.log('\nDependencies:');
    console.log(chalk.yellow('  Production:'), Object.keys(pkg.dependencies || {}).length);
    console.log(chalk.yellow('  Development:'), Object.keys(pkg.devDependencies || {}).length);
    console.log('');
...
async function projectDev() {
  ...
  try {
    ...
    const isPortAvailable = await retryOperation(() => checkPort(3000), 3, 1000, 'checkPort');
    if (!isPortAvailable) {
  ...
  if (!hasEnvFile) {
    writeLog('Warning: No .env file found. Server might not work as expected.', 'WARN');
  }
...
async function validateEnvironment() {
  try {
    ...
    const missingVars = Object.keys(envConfig).filter(key => !process.env[key]);

    if (missingVars.length > 0) {
      writeLog(`Warning: Missing environment variables: ${missingVars.join(', ')}`, 'WARN');
    }
    return true;
  } catch {
    return false;
...
process.on('unhandledRejection', (reason, promise) => {
  ...
  console.error(chalk.red(`Unhandled Rejection: ${reason}, Promise: ${promise}`));
});
...
async function retryOperation(operation, retries = 3, delayMs = 1000, context = '') {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      return await operation();
    } catch (error) {
      const contextMessage = context ? ` in ${context}` : '';
      writeLog(`Attempt ${attempt} failed${contextMessage}: ${error.message}`, 'WARN');
      if (attempt < retries) {
...
async function resetProject() {
  ...
  try {
    ...
    writeLog(`Project reset failed: ${error.message} - Original Error: ${error}`, 'ERROR');
    return false;
  }
}
