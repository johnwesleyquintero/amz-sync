...
const ERROR_PATTERNS = [
  ...
  {
    pattern: /(npm ERR!|yarn error|pnpm error|Cannot find module|Module not found)/i,
    category: ISSUE_CATEGORIES.DEPENDENCY,
  },
  {
    pattern: /(ENOENT|EPERM|EACCES|Failed to read|Failed to write|Cannot write to log file)/i,
    category: ISSUE_CATEGORIES.FILESYSTEM,
  }, // Fallback regex
  {
    pattern: /(next\.config\.js|wescore\.json|Invalid configuration|Failed to load config)/i,
...
class IssueLogger {
  ...
  _determineRotationNumber() {
    let rotationNum = 1;
    while (rotationNum < this.maxLogFiles && fs.existsSync(`${this.logFilePath}.${rotationNum}`)) {
      rotationNum++;
    }
    return rotationNum;
  }
  ...
  _deleteOldestLog() {
    ...
    console.log(`[IssueLogger] Max log files reached. Deleting oldest: ${oldestLog}`);
    try {
  ...
  _shiftExistingLogs(rotationNum) {
    for (let i = rotationNum - 1; i >= 1; i--) {
      try {
        fs.renameSync(`${this.logFilePath}.${i}`, `${this.logFilePath}.${i + 1}`);
      } catch (e) {
  ...
  _logIssue(category, logEntry, issueDetail) {
    if (!this.isInitialized) {
      ...
      if (!this._initializeLogger()) {
        ...
        try {
          process.stderr.write(criticalMsg + JSON.stringify(issueDetail) + '\n');
        } catch {
          /* Intentional no-op */
        }
  ...
  _writeSummaryFile() {
    try {
      ...
      try {
        if (fs.existsSync(this.summaryTmpPath)) fs.unlinkSync(this.summaryTmpPath);
      } catch {
        /* Ignore stderr write failures */
      }
    }
  ...
  logInternalError(error, context = {}) {
    ...
    const { logEntry, issueDetail } = this._formatInternalErrorEntry(error, category, context);
  ...
  logLintResults(eslintResults, context = {}) {
    ...
    for (const result of eslintResults) {
      if (result.errorCount > 0 || result.warningCount > 0) {
        const { logEntry, issueDetail } = this._formatLintResultEntry(result, category, context);
        this._logIssue(category, logEntry, issueDetail); // Use the common internal log method
