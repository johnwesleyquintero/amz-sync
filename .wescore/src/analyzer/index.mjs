...
export function categorizeLogOutput(
  ...
  const compiledErrorPatterns = errorPatterns.map(pattern => new RegExp(pattern, 'i'));
  const compiledWarningPatterns = warningPatterns.map(pattern => new RegExp(pattern, 'i'));
